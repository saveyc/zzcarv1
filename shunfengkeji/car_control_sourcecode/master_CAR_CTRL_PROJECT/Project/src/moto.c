            ...
            if( moto_recv_buff[1] == moto_para[cur_moto_para_num].num )
            {
                if( moto_recv_buff[3] == ( moto_recv_buff[1] ^ moto_recv_buff[2] ) )
                {
                    ...
                    if( moto_ctr_statue == WAIT_RECV_PARA )
                    {
                        ...
                        if(moto_para[cur_moto_para_num].xialiao_postion == 0)
                        {
                            moto_para[cur_moto_para_num].run_en = MOTO_OPT_RUN_EN;
                            moto_para[cur_moto_para_num].xialiao_postion = 0xFFFFFFFF;
                        }
...
void  get_load_num(void)
{
    u8 platform_index = 0;
    //u32 true_or_false = 0;
    uint16_t i, j, tmp;//, cur_postion_tmp = 0;
        ...
        //if( Load_info_t.car_load_info_t[i].car_load_length_u16 != 0 )
        {
            ...
            for(j = 0; j < MOTO_MAX_COUNT; j++)
            {
                if( moto_para[j].xiaoche_num == tmp )
                {
                    ...
                    moto_para[j].xialiao_postion = 0xFFFFFFFF;
                    moto_para[j].load_postion = 0xFFFF;//wcs2CarParaData.load_position[platform_index];
                    moto_para[j].juli = (u8)((1.15*Load_info_t.car_load_info_t[i].car_run_time*10) / 15);
                    moto_para[j].dely = Load_info_t.car_load_info_t[i].car_wait_time;
...
void xialiao_process(void)
{
    ...
    for( i = 0; i < MOTO_MAX_COUNT; i++ )
    {
        ...
        sprintf(debug_info, "moto_cur_pos : %d   \n",  moto_para[i].cur_postion);
        DEBUG_process_udp(debug_info, strlen(debug_info));
#endif
    }
...
//
//        if( (true_or_false == 1)  && (moto_para[i].run_en == MOTO_OPT_NO_CMD) && (moto_para[i].load_postion != 0xffff))
//        {
