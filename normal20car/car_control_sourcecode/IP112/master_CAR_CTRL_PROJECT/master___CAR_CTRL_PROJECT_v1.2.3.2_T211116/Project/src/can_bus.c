...
void can_bus_frame_receive(CanRxMsg rxMsg)
{
    ...
    else if ((func_id == CAN_FUNC_ID_CAR_TEST_TYPE) && (dst_id == 1)) {
        slaver_photo_test_buff[0] = rxMsg.Data[0];
        slaver_photo_test_buff[1] = rxMsg.Data[1];
        slaver_photo_test_buff[2] = rxMsg.Data[2];
        AddSendMsgToQueue(SEND_MSG_SLAVER_PHOTO_TEST_TYPE);
    }
}
