...
{
    funheart.heartack_cnt = 0;
    funheart.heart_exception = 0;
    funheart.twosec = 0;
    funheart.reboot = 0;
}
...
void recv_msg_process(u8 *point)
{
    ...
    case RECV_MSG_WCS2CAR_UNLOAD_RUN_DATA: //下料配置参数 0x1135
        ...
        if( (max_guandian_count_u16 - XIAOCHE_START_NUM ) > 20 )
        {
            MOTO_MAX_COUNT = 20;
        }
        else
        ...
        if (sWCS2CarUnloadRunData.car_num_u16 % 20 == 0) {
            slaverheartmsg.num = (sWCS2CarUnloadRunData.car_num_u16 / 20) - 1;
        }
        else {
            slaverheartmsg.num = (sWCS2CarUnloadRunData.car_num_u16 / 20);
        }
        //zongxian_uart_broadcast(point, head->MSG_TYPE);
        can_bus_send_wcs_msg(point, head->MSG_LENGTH);
        reply_index_error = head->MSG_ID;
    ...
    case RECV_MSG_WCS2CAR_CHECK_CAR_TYPE: //测试小车命令 0x1133
        ...
        can_bus_send_wcs_msg(point, head->MSG_LENGTH);
        break;
    case RECV_SEND_MSG_CAR2WCS_HEART_TYPE_ACK:
        funheart.heartack_cnt = 0;
        break;
...
void send_msg_car2wcs_err_heart(u8* buf, u16* len, u16 type)
{
    u8 sum = 0;
    ...
    buf[6] = 0x0C;
    buf[7] = 0x00;
    buf[11] = funheart.heart_exception;
    funheart.heart_exception--;


    sum = (type & 0xFF) ^ (type >> 8) ^ (buf[12]);
...
void send_msg_car2wcs_online_cmd(u8 *buf, u16 *len, u16 type)
{
    ...
    car2WcsOnlineData.car_version = VERSION(1, 2, 8, 8);//(((1 << 4) | (2)) << 8) | (0 << 4) | ( 1);
...
void fun_send_slaver_photo_test_cmd(u8* buf, u16* len, u16 type)
{
    u16 sendlen = 0;
    u8 sum;
    u8 i = 0;
    ...
    buf[9] = type & 0xFF;
    buf[10] = (type >> 8) & 0xFF;
    buf[11] = slaver_photo_test_buff[0];
    buf[12] = slaver_photo_test_buff[1];
    buf[13] = slaver_photo_test_buff[2];
...
void send_message_to_sever(void)
{
    ...
    case SEND_MSG_CAR2WCS_ONLINE_TYPE:
        ...
        break;
    case SEND_MSG_CAR2WCS_ERR_HEART_TYPE:
        send_msg_car2wcs_err_heart(&(tcp_client_list[0].tcp_send_buf[0]), &(tcp_client_list[0].tcp_send_len), msg_type);
        tcp_client_list[0].tcp_send_en = 1;
        break;
    ...
    case SEND_MSG_BD2PC_SLAVER_PHOTO_CNT:
        ...
        tcp_client_list[0].tcp_send_en = 1;
        break;
     case SEND_MSG_SLAVER_PHOTO_TEST_TYPE:
         fun_send_slaver_photo_test_cmd(&(tcp_client_list[0].tcp_send_buf[0]), &(tcp_client_list[0].tcp_send_len), msg_type);
         tcp_client_list[0].tcp_send_en = 1;
         break;
    ...
    default:
        if( tcp_client_list[0].tcp_client_statue == CLIENT_CONNECT_OK)
        {
            if( heart_dely == 0 )
            {
                ...
                funheart.heartack_cnt++;
                if (funheart.heartack_cnt >= 100) {
                    funheart.heartack_cnt = 100;
                }
            }
        }
        break;
...
void fun_upload_heart_unconnect_process(void)
{
    ...
    if (funheart.heartack_cnt >= 4)
    {
        funheart.heartack_cnt = 0;
        funheart.heart_exception = 30;
        funheart.twosec = 0;
        EthInitStatus = 0;
        tcp_client_list[0].connect_is_ok = 0;
        tcp_client_list[0].tcp_client_statue = CLIENT_DIS_CONNECT;
        InitSendMsgQueue();
        udp_remove(tcp_client_list[0].echoclient_pcb);
        netif_remove(&netif);
...
{
    if (EthInitStatus == 0) return;
    funheart.twosec++;
    if ((funheart.twosec % 1) == 0) {
        funheart.twosec = 0;
        if (funheart.heart_exception != 0) {
            //            funheart.heart_exception--;
            AddSendMsgToQueue(SEND_MSG_CAR2WCS_ERR_HEART_TYPE);
        }
    }
}
...
void vfun_upload_slaver_err_heart_program(void)
{
    ...
    for (i = 0; i < slaverheartmsg.num; i++) {
        if (slaverheartmsg.heartcnt[i] >= 10) {
            slaverheartmsg.heartcnt[i] = 10;
