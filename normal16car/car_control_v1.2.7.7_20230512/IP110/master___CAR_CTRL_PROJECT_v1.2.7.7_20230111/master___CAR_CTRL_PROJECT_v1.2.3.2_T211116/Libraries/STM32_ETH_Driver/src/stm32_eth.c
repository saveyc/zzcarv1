...
ethinit_config  ethinitconfig;
ethinit_data    ethinitdata;

void ethinit_configstruct(void)
{
    ethinitconfig.firststep = 0;
    ethinitconfig.secondstep = 0;
    ethinitconfig.thirdstep = 0;
    ethinitconfig.fourstep = 0;
    ethinitconfig.fivestep = 0;
    ethinitconfig.sixstep = 0;
    ethinitconfig.sevenstep = 0;

    ethinitdata.RegValue = 0;
    ethinitdata.tmpreg = 0;
    ethinitdata.i = 0;
    ethinitdata.hclk = 60000000;
    ethinitdata.timeout = 0;
}
...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
    ...
    if (ethinitconfig.firststep == 0) {
        /*---------------------- ETHERNET MACMIIAR Configuration -------------------*/
  /* Get the ETHERNET MACMIIAR value */
        ethinitdata.tmpreg = ETH->MACMIIAR;
        /* Clear CSR Clock Range CR[2:0] bits */
        ethinitdata.tmpreg &= MACMIIAR_CR_MASK;
        /* Get hclk frequency value */
        RCC_GetClocksFreq(&(ethinitdata.rcc_clocks));
        ethinitdata.hclk = ethinitdata.rcc_clocks.HCLK_Frequency;
        /* Set CR bits depending on hclk value */
        if ((ethinitdata.hclk >= 20000000) && (ethinitdata.hclk < 35000000))
        {
            /* CSR Clock Range between 20-35 MHz */
            ethinitdata.tmpreg |= (uint32_t)ETH_MACMIIAR_CR_Div16;
        }
        else if ((ethinitdata.hclk >= 35000000) && (ethinitdata.hclk < 60000000))
        {
            /* CSR Clock Range between 35-60 MHz */
            ethinitdata.tmpreg |= (uint32_t)ETH_MACMIIAR_CR_Div26;
        }
        else /* ((hclk >= 60000000)&&(hclk <= 72000000)) */
        {
            /* CSR Clock Range between 60-72 MHz */
            ethinitdata.tmpreg |= (uint32_t)ETH_MACMIIAR_CR_Div42;
        }
        /* Write to ETHERNET MAC MIIAR: Configure the ETHERNET CSR Clock Range */
        ETH->MACMIIAR = (uint32_t)ethinitdata.tmpreg;
  ...
  /* Put the PHY in reset mode */
    ...
    if (ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
    {
        ...
        if (ethinitconfig.secondstep == 0) {
            if ((!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_Linked_Status) && (ethinitdata.timeout < PHY_READ_TO)) == 1) {
                ethinitdata.timeout++;
                return 2;
            }

            /* Return ERROR in case of timeout */
            if (ethinitdata.timeout == PHY_READ_TO)
            {
                return ETH_ERROR;
            }
            ...
            ethinitdata.timeout++;
            ethinitconfig.secondstep = 1;
        }

        if (ethinitconfig.thirdstep == 0) {
            /* Wait until the autonegotiation will be completed */
            if ((!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_AutoNego_Complete) && (ethinitdata.timeout < (uint32_t)PHY_READ_TO)) == 1) {
                ethinitdata.timeout++;
                return 2;
            }
            ...
            ethinitdata.RegValue = ETH_ReadPHYRegister(PHYAddress, PHY_SR);

            /* Configure the MAC with the Duplex Mode fixed by the autonegotiation process */
            if ((ethinitdata.RegValue & PHY_Duplex_Status) != (uint32_t)RESET)
            {
            ...
            if (ethinitdata.RegValue & PHY_Speed_Status)
            {
    ...
    ethinitdata.tmpreg = ETH->MACCR;
    /* Clear WD, PCE, PS, TE and RE bits */
    ethinitdata.tmpreg &= MACCR_CLEAR_MASK;
    ...
    ethinitdata.tmpreg |= (uint32_t)(ETH_InitStruct->ETH_Watchdog |
    ...
    ETH->MACCR = (uint32_t)ethinitdata.tmpreg;
    ...
    ethinitdata.tmpreg = ETH->MACFCR;
    /* Clear xx bits */
    ethinitdata.tmpreg &= MACFCR_CLEAR_MASK;
    ...
    ethinitdata.tmpreg |= (uint32_t)((ETH_InitStruct->ETH_PauseTime << 16) |
    ...
    ETH->MACFCR = (uint32_t)ethinitdata.tmpreg;
    ...
    ethinitdata.tmpreg = ETH->DMAOMR;
    /* Clear xx bits */
    ethinitdata.tmpreg &= DMAOMR_CLEAR_MASK;
    ...
    ethinitdata.tmpreg |= (uint32_t)(ETH_InitStruct->ETH_DropTCPIPChecksumErrorFrame |
    ...
    ETH->DMAOMR = (uint32_t)ethinitdata.tmpreg;
