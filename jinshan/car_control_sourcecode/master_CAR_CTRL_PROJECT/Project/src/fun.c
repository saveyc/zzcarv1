...
void vpcfun_init_sys_item(void)
{
...
void recv_msg_process(u8 *point)
{
    ...
    case RECV_MSG_WCS2CAR_UNLOAD_RUN_DATA: //下料配置参数 0x1135
        ...
        if (sWCS2CarUnloadRunData.car_num_u16 % 16 == 0) {
            slaverheartmsg.num = sWCS2CarUnloadRunData.car_num_u16 / 16 - 1;
        }
        else {
            slaverheartmsg.num = sWCS2CarUnloadRunData.car_num_u16 / 16;
        }
        //zongxian_uart_broadcast(point, head->MSG_TYPE);
        can_bus_send_wcs_msg(point, head->MSG_LENGTH);
        reply_index_error = head->MSG_ID;
...
void fun_send_car_para_cmd(u8* buf, u16* len, u16 type)
{
    u16 sendlen = 0;
    u8 sum;
    u8 i = 0;

    sendlen = 11 + 2 + car_test_para_buff[1];

    buf[9] = type & 0xFF;
    buf[10] = (type >> 8) & 0xFF;

    for (i = 0; i < sendlen - 11; i++) {
        buf[11 + i] = car_test_para_buff[i];
    }
    sum = buf[9];
...
void fun_send_car_all_msg_cmd(u8* buf, u16* len, u16 type)
{
    u16 sendlen = 0;
    u8 sum;
    u8 i = 0;

    sendlen += 2 + car_test_allmsg_buff[1];
    sendlen += 1 + car_test_allmsg_buff[sendlen];
    sendlen += 1 + car_test_allmsg_buff[sendlen];
    sendlen += 1 + car_test_allmsg_buff[sendlen];
    sendlen += 11;
    ...
    for (i = 0; i < sendlen - 11; i++) {
        buf[11 + i] = car_test_allmsg_buff[i];
    }
    sum = buf[9];
...
void fun_send_car_para_errack_msg_cmd(u8* buf, u16* len, u16 type)
{
    u16 sendlen = 0;
    u8 sum;
    u8 i = 0;
    ...
    for (i = 0; i < sendlen - 11; i++) {
        buf[11 + i] = car_test_para_errack_buff[i];
    }
    sum = buf[9];
...
void send_msg_car2wcs_online_cmd(u8 *buf, u16 *len, u16 type)
{
    ...
    car2WcsOnlineData.car_version = VERSION(1, 2, 8, 9);//(((1 << 4) | (2)) << 8) | (0 << 4) | ( 1);
...
void send_message_to_sever(void)
{
    ...
    case SEND_MSG_CAR2WCS_ONLINE_TYPE:
        ...
        break;
    case SEND_CAR_PARA_MSG_TYPE:
        fun_send_car_para_cmd(&(tcp_client_list[0].tcp_send_buf[0]), &(tcp_client_list[0].tcp_send_len), msg_type);
        tcp_client_list[0].tcp_send_en = 1;
        break;
    case SEND_CAR_TEST_ALL_MSG_TYPE:
        fun_send_car_all_msg_cmd(&(tcp_client_list[0].tcp_send_buf[0]), &(tcp_client_list[0].tcp_send_len), msg_type);
        tcp_client_list[0].tcp_send_en = 1;
        break;
    case SEND_CAR_PARA_ERRACK_TYPE:
        fun_send_car_para_errack_msg_cmd(&(tcp_client_list[0].tcp_send_buf[0]), &(tcp_client_list[0].tcp_send_len), msg_type);
        tcp_client_list[0].tcp_send_en = 1;
        break;
