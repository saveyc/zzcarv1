...
u16 carparaindex = 0;
u16 carparaackindex = 0;
u16 carparaackcheckindex = 0;
u16 carrunindex = 0;

u8  moto_para_ack_buff[5];

#define  canframebuffsize   50
sCanFrameExt    canframequeuebuff[canframebuffsize];
sCanFrameQueue   canframequeue;

u8  can_send_buff[CAN_SEND_DATA_LEN];
u16 can_send_tot = 0;
...
void can_bus_reply_ack(u16 fun_id)
{
    ...
    if(  ((XIAOCHE_START_NUM +16 -1) < max_guandian_count_u16)
...
void vcanbus_send_cartest_process(void)
{
    sCanFrameExt canTxMsg;
    u8 result = 0;
    ...
    canTxMsg.extId.seg_polo = CAN_SEG_POLO_NONE;
    canTxMsg.extId.seg_num = 0;
    canTxMsg.extId.func_id = CAN_FUNC_ID_CAR_TEST;
    canTxMsg.extId.mac_id = XIAOCHE_GROUP_NUM;
    canTxMsg.extId.dst_id = 1;
    canTxMsg.data[0] = carparaindex & 0xFF;
    canTxMsg.data[1] = (carparaindex >> 8) & 0xFF;
    canTxMsg.data[2] = carparaackindex & 0xFF;
    canTxMsg.data[3] = (carparaackindex >> 8) & 0xFF;
    canTxMsg.data[4] = carparaackcheckindex & 0xFF;
    canTxMsg.data[5] = (carparaackcheckindex >> 8) & 0xFF;
    canTxMsg.data[6] = carrunindex & 0xFF;
    canTxMsg.data[7] = (carrunindex >> 8) & 0xFF;
    canTxMsg.data_len = 8;

//    result = can_bus_send_one_test_frame(canTxMsg);

    carparaindex = 0;
    carparaackcheckindex = 0;
    carparaackindex = 0;
    carrunindex = 0;
}

void vcanbus_send_carpara_process(void)
{
    sCanFrameExt canTxMsg;
    u8 result = 0;
...
void vcanbus_send_carpara_ack_process(void)
{
    sCanFrameExt canTxMsg;
    u8 result = 0;
    u16 i = 0;
    ...
    canTxMsg.extId.seg_polo = CAN_SEG_POLO_NONE;
    canTxMsg.extId.seg_num = 0;
    canTxMsg.extId.func_id = CAN_FUNC_ID_MOTO_ACK;
    canTxMsg.extId.mac_id = XIAOCHE_GROUP_NUM;
    canTxMsg.extId.dst_id = 1;
    canTxMsg.data[0] = moto_para_ack_buff[0] & 0xFF;
    canTxMsg.data[1] = moto_para_ack_buff[1] & 0xFF;
    canTxMsg.data[2] = moto_para_ack_buff[2] & 0xFF;
    canTxMsg.data[3] = moto_para_ack_buff[3] & 0xFF;
    canTxMsg.data[4] = moto_para_ack_buff[4] & 0xFF;
    canTxMsg.data_len = 5;
