...
void moto_ready_recv_process(void)
{
    ...
    moto_recv_count = 0;
    moto_recv_statue = 0;
    carparaackindex |= (1 << cur_moto_para_num);
}
/************************************
...
{
    u8 i = 0;
    if (moto_recv_count != 0) {
        carparaackcheckindex |= (1 << cur_moto_para_num);
    }
            ...
            if( moto_recv_buff[1] == moto_para[cur_moto_para_num].num )
            {
                if( moto_recv_buff[3] == ( moto_recv_buff[1] ^ moto_recv_buff[2] ) )
                {
                    ...
                    if( moto_ctr_statue == WAIT_RECV_PARA )
                    {
                        ...
                        for (i = 0; i < 5; i++) {
                            moto_para_ack_buff[i] = 0;
                        }
                        moto_para_ack_buff[0] = moto_recv_count;
                        for (i = 0; i < moto_para_ack_buff[0]; i++) {
                            moto_para_ack_buff[1 + i] = moto_recv_buff[i];
                        }
                        vcanbus_send_carpara_ack_process();
